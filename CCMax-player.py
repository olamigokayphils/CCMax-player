...
from PyQt5.QtWidgets import (QApplication, QFileDialog, QHBoxLayout, QLineEdit,
                             ...
                             QWidget, QShortcut, QMenu, QAction, QMainWindow)
import sys
import os
import subprocess
# QT_DEBUG_PLUGINS
...
class VideoPlayer(QWidget):
    ...
    def __init__(self, aPath, parent=None):
        ...
        self.shortcut = QShortcut(QKeySequence(
            Qt.ShiftModifier + Qt.Key_Left), self)
    ...
    def suspend_screensaver(self):
        ...
        proc = subprocess.Popen(
            'gsettings set org.gnome.desktop.screensaver idle-activation-enabled false', shell=True)
        proc.wait()

    def resume_screensaver(self):
        'resume linux screensaver'
        proc = subprocess.Popen(
            'gsettings set org.gnome.desktop.screensaver idle-activation-enabled true', shell=True)
        proc.wait()
    ...
    def volumeUp(self):
        ...
        self.volumeSlider.setSliderPosition(
...
class VideoScreen(QMainWindow):
    def __init__(self):
        ...
        self.customContextMenuRequested[QPoint].connect(
            self.player_widget.contextMenuRequested)
        # self.hideSlider()
        ...
        if len(sys.argv) > 1:
            ...
            if sys.argv[1].startswith("http"):
                self.myurl = sys.argv[1]
                self.playFromURL()
            else:
                self.loadFilm(sys.argv[1])

        self.screen_ui()

    def screen_ui(self):
        menus = self.menuBar()
        xmedia = menus.addMenu("Media")
        xsubtitle = menus.addMenu("Subtitle")
        xtools = menus.addMenu("Tools")
        xhelp = menus.addMenu("Help")

        load_media = QAction("Open File...", self)
        load_media.setShortcut("Ctrl+O")
        load_media.triggered.connect(self.player_widget.openFile)

        quit_media = QAction("Quit", self)
        quit_media.setShortcut("Ctrl+Q")
        quit_media.triggered.connect(self.player_widget.handleQuit)

        xmedia.addAction(load_media)
        xmedia.addAction(quit_media)
...
if __name__ == '__main__':
    ...
    player = VideoScreen()
    player.setWindowIcon(
        QIcon.fromTheme("multimedia-video-player"))
    player.setWindowTitle("CCMax Player")
...
fix background color centre widget
"""
